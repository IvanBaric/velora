<div>
    <flux:modal wire:model="isOpen" flyout variant="floating" class="w-full max-w-xl space-y-6">
        <div class="space-y-5 pt-6 pe-10">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                ...
                @if ($rolesAndPermissionsAvailable)
                    <flux:button wire:click="createRole" variant="primary" icon="plus" class="w-full sm:w-auto">{{ __('Nova uloga') }}</flux:button>
                @else
            ...
            <div class="grid gap-3 sm:grid-cols-2">
                <div class="admin-inset-panel p-4">
        ...
        <div class="space-y-2">
            @foreach ($roles as $role)
                ...
                <article class="admin-list-card group p-4">
    ...
    <flux:modal wire:model="isFormOpen" variant="floating" class="m-0 h-[100dvh] max-h-[100dvh] w-screen max-w-none overflow-hidden rounded-none p-0">
        <form wire:submit="save" wire:loading.class="admin-panel-content-loading" wire:target="save" class="relative flex h-[100dvh] max-h-[100dvh] flex-col overflow-hidden">
            ...
            <div class="min-h-0 flex-1 overflow-y-auto bg-zinc-50/70 p-6 dark:bg-zinc-950">
                <div class="mx-auto grid max-w-7xl items-start gap-8 lg:grid-cols-[360px_minmax(0,1fr)]">
                    <aside class="space-y-4 lg:sticky lg:top-0 lg:self-start">
                        <section class="rounded-lg bg-white p-5 ring-1 ring-zinc-950/5 dark:bg-zinc-950 dark:ring-white/10">
                            <div class="mb-5 flex items-center gap-3">
                                <div class="flex size-9 shrink-0 items-center justify-center rounded-lg bg-accent/10 text-accent-content ring-1 ring-accent/15 dark:bg-accent/15 dark:text-accent-content dark:ring-accent/25">
                            ...
                            <div class="space-y-4">
                                <flux:input wire:model="name" label="{{ __('Naziv') }}" placeholder="{{ __('Urednik, Podrška, Naplata...') }}" clearable :disabled="$isReadOnly" data-required />

                                @if ($isReadOnly)
                                    <div class="admin-inset-panel px-4 py-3 text-sm text-zinc-600 dark:text-zinc-300">
                    ...
                    <div class="space-y-4">
                        @forelse ($permissions as $permissionGroup)
                            <details class="group rounded-lg bg-white ring-1 ring-zinc-950/5 transition duration-150 ease-out open:ring-zinc-950/10 dark:bg-zinc-950 dark:ring-white/10 dark:open:ring-white/15">
                        ...
                        @empty
                            <div class="rounded-lg bg-white p-8 text-center ring-1 ring-zinc-950/5 dark:bg-zinc-950 dark:ring-white/10">
                                <div class="mx-auto flex size-11 items-center justify-center rounded-lg bg-zinc-50 text-zinc-400 ring-1 ring-zinc-950/5 dark:bg-zinc-900 dark:text-zinc-500 dark:ring-white/10">
    ...
    <flux:modal wire:model="isDeleteConfirmOpen" variant="floating" class="space-y-6">
        <div class="flex items-start gap-4">
            <div class="flex size-11 shrink-0 items-center justify-center rounded-lg bg-red-50 text-red-600 ring-1 ring-red-200 dark:bg-red-500/10 dark:text-red-300 dark:ring-red-400/20">
        ...
        @if ($pendingDeleteUserCount > 0)
            ...
            <div class="rounded-lg bg-amber-50 p-4 text-sm text-amber-800 ring-1 ring-amber-200 dark:bg-amber-500/10 dark:text-amber-200 dark:ring-amber-400/20">
        ...
        @else
            <div class="admin-inset-panel p-4 text-sm text-zinc-600 dark:text-zinc-300">
